...
public class CustomerController {
	...
	public ResponseEntity<Customer> findCustomerById(
		...
		return ResponseEntity.status(HttpStatus.OK).body(
				RestPreconditions.checkFound(customerService
						.getCustomerById(id)));
	...
	public ResponseEntity<Customer> createCustomer(
			@Valid @RequestBody @ApiParam(value = "customer") Customer resource) {

		return ResponseEntity.status(HttpStatus.CREATED).body(
				customerService.createCustomer(resource));
	}

	@PutMapping(value = "customer/{id}")
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Customer> updateCustomer(@PathVariable("id") long id,
			@Valid @RequestBody Customer resource) {

		RestPreconditions.checkNotNull(customerService.getCustomerById(resource
				.getId()));
		return new ResponseEntity<Customer>(
				customerService.updateCustomer(resource), HttpStatus.OK);
	}

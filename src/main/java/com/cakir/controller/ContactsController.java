...
public class ContactsController {
	...
	@PostMapping(value = "/")
	public ResponseEntity<Contacts> createContact(@Valid @RequestBody Contacts contact) {
		
		return ResponseEntity.status(HttpStatus.CREATED).body(contactsService.createContact(contact));
	}
	
	
	@PutMapping(value = "/{id}")
	public ResponseEntity<Contacts> updateContact(@PathVariable("id") long id,@Valid @RequestBody Contacts contact) {
		
		RestPreconditions.checkFound(contactsService.getContactById(id));
		return ResponseEntity.status(HttpStatus.OK).body(contactsService.updateContact(contact));
	}
	...
	public ResponseEntity<List<Contacts>> searchContactsByNachname(@PathVariable("nachname") String nachname) {
